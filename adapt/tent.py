...
class TENT:
    ...
    def __init__(self, backbone, lr, classes, steps=10,
    ...
    def evaluate(self, x, classes, vision_outputs=(-1,)):
        ...
        """
        logits, _, _ = self.model(x, self.text_x[-1], True, vision_outputs=vision_outputs, interpolate=True) # (#template, batch_size, #classes, H, W)
        logits = logits[0]
    ...
    def perform_adaptation(self, x, classes, vision_outputs=(-1,)):
        ...
        for iter in range(self.steps):
            if self.average_type == 'loss':
                ...
                logits, _, _ = self.model(x, self.text_x[:-1], True, vision_outputs=vision_outputs, interpolate=self.interpolate)  # (#template, batch_size, #classes, H, W)
